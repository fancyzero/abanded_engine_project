...
inline static bool load_file( const TCHAR* path, TCHAR** buffer, unsigned long* size, unsigned long tempsize = 4096 )
{
	...
	*buffer = 0;
	...
	WORD header;
	size_t ret = fread_s( &header, 2, 1, 2, f );
	fclose( f );
	bool is_unicode = ( ret == 2 && header == 0XFEFF )	;
	if 	( is_unicode )
	...
	fseek( f, 0, SEEK_SET );
	TCHAR* _buf;
	_buf = ( TCHAR* )( new char[filesize] );
	size_t readed = ::fread( _buf, 1, filesize, f );
	*size = ( unsigned int )readed;
	...
	if ( !is_unicode )
	{
		*buffer = new TCHAR[readed+1];
		size_t num_converted = 0;
		errno_t err = ::mbstowcs_s( &num_converted, *buffer, readed+1, (char*)_buf, readed );
		delete _buf;
		if ( err != 0 )
			return false;	
	}
...
public:
	...
	xml_attribute& operator=( const String& rhs ) { value( rhs.c_str() ); return *this; }
	//<summary>Set attribute to string.</summary>
	//<param name="rhs">Value string to set.</param>
	//<returns>Reference to xml_attribute.</returns>
...
public:
	...
	String getvalue() { return *this; } // Access the attribute value. uses operator String(). NF 2 Mar 2003
#ifdef PUGOPT_NONSEG
...
public:
	...
	bool has_name( const String& name ) const { return has_name( name.c_str() ); } //Node is named 'name'.
	bool has_attribute( const String& name ) const { return has_attribute( name.c_str() ); } //Node has an attribute named 'name'.
#ifdef PUGOPT_NONSEG
...
// inline bool matches_attribute_name_value(const TCHAR* value,xml_attribute_struct* attr) const { return (strcmpwild(value,attr->value)==0); } //The given attribute is valued 'value'.
	...
	xml_attribute attribute( const String& name ) { return attribute( name.c_str() ); }
	//<summary>Access or create the attribute having 'name'.</summary>
	//<param name="name">Name of attribute to access/create.</param>
	//<returns>Reference to xml_attribute wrapper.</returns>
...
public:
	...
	xml_attribute_struct* mapto_attribute_ptr( const String& name ) const { return mapto_attribute_ptr( name.c_str() ); }
	...
	xml_node_struct* mapto_child_ptr( const String& name ) const { return mapto_child_ptr( name.c_str() ); }
	...
	//<returns>Index of child, or -1 if not found.</returns>
	//<remarks>Implement your own hash table if you have a great many children.</remarks>
	int mapto_child_idx( const String& name ) const { return mapto_child_idx( name.c_str() ); }
...
public:
	...
	void all_elements_by_name( const String& name, pointer_array& found ) const { all_elements_by_name( name.c_str(), found ); }
	...
	xml_node first_element_by_name( const String& name, const xml_node_type type = node_null ) const { return first_element_by_name( name.c_str(), type ); }
	...
	//<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
	xml_node first_element_by_value( const String& name, const String& value ) const { return first_element_by_value( name.c_str(), value.c_str() ); }
	...
	//<returns>Valid xml_node if such element named 'name' is found.</returns>
	//<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
	xml_node first_element_by_attribute( const String& name, const String& attr_name, const String& attr_value, const bool bDescend = true )const { return first_element_by_attribute( name.c_str(), attr_name.c_str(), attr_value.c_str(), bDescend ); }
	...
	//<remarks>If matching co-node was found, '_root' points thereto.</remarks>
	bool moveto_first_sibling( const String& name ) { return moveto_first_sibling( name.c_str() ); }
	...
	//<remarks>If matching sub-node was found, '_root' points thereto.</remarks>
	bool moveto_child( const String& name ) { return moveto_child( name.c_str() ); }
	...
	bool moveto_next_sibling( const String& name ) { return moveto_next_sibling( name.c_str() ); }
	...
	xml_node first_element_by_path( const String& path, const String& delimiter = _T( "/" ) ) { return first_element_by_path( path.c_str(), delimiter.c_str() ); }
...
public:
	...
	//<returns>Success.</returns>
	bool name( const String& new_name ) { return name( new_name.c_str() ); }
	...
	//<returns>Success.</returns>
	bool value( const String& new_value ) { return value( new_value.c_str() ); }
	...
	//<returns>Success.</returns>
	bool remove_attribute( const String& name ) { return remove_attribute( name.c_str() ); }
	...
	xml_attribute append_attribute( const String& name, const String& value ) { return append_attribute( name.c_str(), value.c_str() ); }
