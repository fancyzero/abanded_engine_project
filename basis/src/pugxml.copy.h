...
// NF 31 Mar 2003  Added String xml_to_text( const String& sxml_pcdata ).
...
inline static unsigned long lib_variant() {
	return PUGAPI_INTERNAL_VARIANT;
}
...
inline static unsigned long lib_version() {
	return PUGAPI_INTERNAL_VERSION;
}
...
#ifdef PUGOPT_NONSEG
	inline bool matches_attribute_name_value( const TCHAR* pvalue, const unsigned int valuelen ) const {
		return ( valuelen == value_size && _tcsncmp( pvalue, value, value_size ) == 0 );    // This attribute is valued 'value'. NF 28 Mar 2003
	}
#else
	inline bool matches_attribute_name_value( const TCHAR* pvalue ) const {
		return ( strcmpwild( pvalue, value ) == 0 );    // This attribute is valued 'value'. NF 28 Mar 2003
	}
#endif
}
...
typedef struct t_xml_node_struct
{
	...
	// NF 10 Mar 2003
	inline bool type_document() const {
		return ( this == parent && type == node_document );  // Node is tree root.
	}
	inline bool type_element() const {
		return ( type == node_element );  // Node is an element. @since 25 Jun 2003
	}
	inline bool type_pcdata() const {
		return ( type == node_pcdata );  // Node is PCDATA. @since 25 Jun 2003
	}
	inline unsigned int childcount() const {
		return this->children;    // Access node's child count.
	}
	inline t_xml_node_struct* getparent() const {
		return parent;    // Access parent
	}
	inline t_xml_node_struct* get_parent() const {
		return type_document() ? NULL : parent;    // Access parent, return NULL at root. NF 17 Apr 2003
	}
	t_xml_node_struct* getchild( unsigned int i ) const {
		return ( i < children ) ? child[i] : NULL;  //Access child node struct at subscript
	}
	t_xml_node_struct* firstchild() const {
		return children ? child[0] : NULL;    //Access first child node struct
	}
	t_xml_node_struct* lastchild() const {
		return children ? child[children-1] : NULL;    //Access last child node struct
	}
#ifdef PUGOPT_NODE_FLAGS
	inline bool is_expanded() const {
		return ( flags & node_expanded ) != 0;  // Node is expanded.
	}
	inline bool is_selected() const {
		return ( flags & node_selected ) != 0;  // Node is selected.
	}
	inline bool is_display_root() const {
		return ( flags & node_display_root ) != 0;  // Node is the display root.
	}
	inline void expand( const bool bexpand ) {
		if ( bexpand )( flags |= node_expanded );   // set/reset the node is expanded state.
		else ( flags &= ~node_expanded );
	}
	inline void select( const bool bselect ) {
		if ( bselect )( flags |= node_selected );   // set/reset the node is selected state.
		else ( flags &= ~node_selected );
	}
	inline void set_display_root( const bool bset ) {
		if ( bset )( flags |= node_display_root );   // set/reset the node is display root state.
		else ( flags &= ~node_display_root );
	}
	inline unsigned int get_visible_children() const {
		return visible_children;    // count of visible children, including itself.
	}
	inline unsigned int set_visible_children( const unsigned int _visible_children ) {
		return visible_children = _visible_children;
	}
	inline unsigned int inc_visible_children( const unsigned int _visible_children ) {
		return visible_children += _visible_children;
	}
#endif
}
...
inline static bool strwnorm( TCHAR** s )
{
	...
	for ( long i = 1; i < n; ++i ) //For each character, starting at offset 1.
	{
		if (( *s )[i] < _T( '!' ) ) //Whitespace-like.
		{
			...
			norm[j] = ( *s )[i];  //Not whitespace, so just copy over.
			++j;
		}
	}
...
inline static bool load_file( const TCHAR* path, TCHAR** buffer, unsigned long* size, unsigned long tempsize = 4096 )
{
	...
	*buffer = 0;
	FILE* f;
	f = ::_wfopen( path, _T( "r,ccs=UNICODE" ) );
	if ( f == NULL )
		return false;
	fseek( f, 0, SEEK_END );
	long filesize = ftell( f );
	fseek( f, 0, SEEK_SET );
	*buffer = ( TCHAR* )( new char[filesize] );
	size_t readed = fread( *buffer, 1, filesize, f );
	fclose( f );
	*size = ( unsigned int )readed;
	if ( readed != filesize )
		return false;
	return true;
}
...
public:
	bool empty() const {
		return ( _size == 0 );  //True if there is no data in the array.
	}
	void remove_all() {
		_size = 0;    //Remove all data elements from the array.
	}
	...
	unsigned int size() {
		return _size;    //Count data elements in the array.
	}
	virtual void* at( unsigned int i ) {
		if ( i < _size ) return _data[i];  //Access element at subscript, or NULL if overflow.
		else return NULL;
	}
...
public:
	...
	const TCHAR* depth() const {
		return ( _inch && _stac ) ? _stac : _T( "" );
	}
};
...
public:
	virtual long subscript() {
		return _sscr;    //Get subscript value;
	}
	virtual void subscript( long new_subscript ) {
		_sscr = new_subscript;    //Set subscript value;
	}
public:
	virtual xml_iterator& operator=( const xml_iterator& rhs ) {
		_vref = rhs._vref;    //Assignment.
		_sscr = rhs._sscr;
		return *this;
	}
	virtual bool operator==( const xml_iterator& rhs ) {
		return ( _sscr == rhs._sscr );  //True if this is equal to RHS.
	}
	virtual bool operator!=( const xml_iterator& rhs ) {
		return ( _sscr != rhs._sscr );  //True if this is not equal to RHS.
	}
	virtual bool operator<( const xml_iterator& rhs ) {
		return ( _sscr < rhs._sscr );  //True if this subscript is less than RHS.
	}
	virtual bool operator>( const xml_iterator& rhs ) {
		return ( _sscr > rhs._sscr );  //True if this subscript is greater than RHS.
	}
	virtual bool operator<=( const xml_iterator& rhs ) {
		return ( _sscr <= rhs._sscr );  //True if this subscript is less than or equal to RHS.
	}
	virtual bool operator>=( const xml_iterator& rhs ) {
		return ( _sscr >= rhs._sscr );  //True if this subscript is greater than or equal to RHS.
	}
	virtual xml_iterator& operator++() {
		_sscr++;    //Increment the iterator (subscript).
		return *this;
	}
	virtual xml_iterator& operator--() {
		_sscr--;    //Decrement the iterator (subscript).
		return *this;
	}
...
public:
	virtual void push() {
		++_deep;    //Increment node depth.
	}
	virtual void pop() {
		--_deep;    //Decrement node depth.
	}
	virtual long depth() const {
		return ( _deep > 0 ) ? _deep : 0;  //Access node depth.
	}
public:
...
public:
	...
	virtual ~xml_attribute() {
		if ( _delete_attr ) delete _attr;   //Destructor.
	}
//Operators
public:
	...
	xml_attribute& operator=( const xml_attribute& r ) {
		_attr = r._attr;    //Assign internal pointer.
		return *this;
	}
	bool operator==( const xml_attribute& r ) {
		return ( _attr == r._attr );  //Compare internal pointer.
	}
	bool operator!=( const xml_attribute& r ) {
	...
	//<summary>Cast attribute value as String. If not found, return empty.</summary>
	//<returns>The String attribute value, or empty.</returns>
	...
	xml_attribute& operator=( const String& rhs ) {
		value( rhs.c_str() );
		return *this;
	}
	//<summary>Set attribute to string.</summary>
	//<param name="rhs">Value string to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator=( const TCHAR* rhs ) {
		if ( rhs ) value( rhs );
		return *this;
	}
	...
	xml_attribute& operator=( const long rhs )
	{
		...
		_ltot( rhs, temp, 10 );    // NF changed from _stprintf 5 Mar 2003
		value( temp );
		return *this;
	}
	...
	//<summary>Right-shift attribute value to String.</summary>
	//<param name="rhs">Reference to String to set.</param>
	//<returns>Reference to xml_attribute.</returns>
	xml_attribute& operator>>( String& rhs )
	{
#ifdef PUGOPT_NONSEG
	...
	friend long& operator<<( long& lhs, xml_attribute& rhs ) {
		lhs = ( long )rhs;
		return lhs;
	}
	...
	friend double& operator<<( double& lhs, xml_attribute& rhs ) {
		lhs = ( double )rhs;
		return lhs;
	}
	...
	friend bool& operator<<( bool& lhs, xml_attribute& rhs ) {
		lhs = ( bool )rhs;
		return lhs;
	}
	...
	//<returns>Reference to xml_attribute.</returns>
	friend xml_attribute& operator<<( xml_attribute& lhs, const long rhs ) {
		lhs = rhs;
		return lhs;
	}
	...
	//<returns>Reference to xml_attribute.</returns>
	friend xml_attribute& operator<<( xml_attribute& lhs, const double& rhs ) {
		lhs = rhs;
		return lhs;
	}
	...
	//<returns>Reference to xml_attribute.</returns>
	friend xml_attribute& operator<<( xml_attribute& lhs, const bool& rhs ) {
		lhs = rhs;
		return lhs;
	}
public:
	bool empty() const {
		return ( _attr == NULL );  //True if the internal xml_attribute_struct pointer is NULL.
	}
	bool has_name() const {
		return ( !empty() && _attr->name );  //True if the attribute has a name.
	}
	bool has_value() const {
		return ( !empty() && _attr->value );  //True if the attribute has a value.
	}
#ifdef PUGOPT_NONSEG
	bool has_name( const TCHAR* name ) const {
		return ( name && !empty() && has_name() && _tcsncmp( _attr->name, name, _attr->name_size ) == 0 );    //Is named 'name'.
	}
	bool has_value( const TCHAR* value ) const {
		return ( value && !empty() && has_value() && _tcsncmp( _attr->value, value, _attr->value_size ) == 0 );    //Has value 'value'.
	}
#else
	bool has_name( const TCHAR* name ) const {
		return ( name && !empty() && has_name() && _tcscmp( _attr->name, name ) == 0 );    //Is named 'name'.
	}
	bool has_value( const TCHAR* value ) const {
		return ( value && !empty() && has_value() && _tcscmp( _attr->value, value ) == 0 );    //Has value 'value'.
	}
#endif
public:
	const TCHAR* name() const {
		return ( !empty() && _attr->name ) ? _attr->name : _T( "" );    //Access the attribute name.
	}
	...
	const unsigned int name_size() {
		return ( !empty() ) ? _attr->name_size : 0;  //Access the attribute name length (for PUGOPT_NONSEG).
	}
#endif
	...
	const TCHAR* value() {
		return ( !empty() ) ? _attr->value : _T( "" );    //Access the attribute value.
	}
	String getvalue() {
		return *this;    // Access the attribute value. uses operator String(). NF 2 Mar 2003
	}
#ifdef PUGOPT_NONSEG
	const unsigned int value_size() {
		return ( !empty() ) ? _attr->value_size : 0;  //Access the attribute name length (for PUGOPT_NONSEG).
	}
#endif
...
public:
	forward_class() : _obj( NULL ) {
		_obj = new TYPE();    //Default constructor.
	}
	forward_class( const TYPE& r ) : _obj( NULL ) {
		_obj = new TYPE( r );  //Copy constructor.
	}
	virtual ~forward_class() {
		if ( _obj ) delete _obj;  //Destructor.
	}
public:
	TYPE& operator*() {
		return *_obj;    //Dereference to the class.
	}
	TYPE* operator->() {
		return _obj;    //Class member selection.
	}
	operator TYPE() {
		return *_obj;    //Cast as class type.
	}
	operator TYPE&() {
		return *_obj;    //Cast as class type reference.
	}
	operator TYPE*() {
		return _obj;    //Cast as class type pointer.
	}
	forward_class& operator=( const forward_class& rhs ) {
		return *this;    // 7 Oct 2003 See: http://www.codeproject.com/soap/pugxml.asp?msg=620173#xx620173xx
	}
};
...
public:
	...
	xml_node( xml_node_struct* p ): _root( p ) {
		memset( &_dummy, 0, sizeof( xml_node_struct ) );
	}
...
public:
	...
	sibling_iterator siblings_begin() {
		if ( has_siblings() ) return sibling_iterator( _root->parent, 0 );
	...
	sibling_iterator siblings_end() {
		if ( has_siblings() ) return sibling_iterator( _root->parent, _root->parent->children );
		return sibling_iterator();
	}
...
public:
	...
	operator xml_node_struct*() const {
		return _root;    //Cast as xml_node_struct pointer.
	}
	operator void*() const {
		return ( void* )_root;  //Cast root as void*.
	}
	xml_node& operator=( const xml_node& r ) {
		_root = r._root;    //Assign to xml_node_struct pointer.
		return *this;
	}
	bool operator==( const xml_node& r ) {
		return ( _root == r._root );  //True if this has the same internal xml_node_struct pointer value.
	}
	xml_node operator[]( unsigned int i ) {
		return child( i );  //Access the child at subscript.
	}
...
public:
	...
	bool empty() const {
		return ( _root == 0 || _root->type == node_null );  //Node pointer is null, or type is node_null. Same as type_null.
	}
	bool type_null() const {
		return empty();    //Node pointer is null, or type is node_null. Same as empty.
	}
	bool type_document() const {
		return ( _root && _root == _root->parent && _root->type == node_document );  //Node is tree root.
	}
	bool type_element() const {
		return ( _root && _root->type == node_element );  //Node is an element.
	}
	bool type_comment() const {
		return ( _root && _root->type == node_comment );  //Node is a comment.
	}
	bool type_pcdata() const {
		return ( _root && _root->type == node_pcdata );  //Node is PCDATA.
	}
	bool type_cdata() const {
		return ( _root && _root->type == node_cdata );  //Node is CDATA.
	}
	bool type_include() const {
		return ( _root && _root->type == node_include );  //Node is INCLUDE.
	}
	bool type_pi() const {
		return ( _root && _root->type == node_pi );  //Node is a processing instruction.
	}
	bool type_doctype() const {
		return ( _root && _root->type == node_doctype );  //Node is DOCTYPE.
	}
	bool type_dtd_item() const {
		return ( _root && _root->type > node_doctype );  //Node is NODE_DTD_*.
	}
	bool type_dtd_attlist() const {
		return ( _root && _root->type == node_dtd_attlist );  //Node is node_dtd_attlist.
	}
	bool type_dtd_element() const {
		return ( _root && _root->type == node_dtd_element );  //Node is node_dtd_element.
	}
	bool type_dtd_entity() const {
		return ( _root && _root->type == node_dtd_entity );  //Node is node_dtd_entity.
	}
	bool type_dtd_notation() const {
		return ( _root && _root->type == node_dtd_notation );  //Node is node_dtd_notation.
	}
...
public:
	...
	bool has_value() const {
		return ( !empty() && _root->value != 0 );  //Node has data (comment, CDATA or PCDATA).
	}
	bool has_child_nodes() const {
		return ( !empty() && children() > 0 );  //Node has 1 or more children.
	}
	bool has_attributes() const {
		return ( !empty() && attributes() > 0 );  //Node has 1 or more attributes.
	}
	bool has_siblings() const {
		return ( !empty() && siblings() > 0 );  //Node has one or more siblings.
	}
	bool has_name() const {
		return ( !empty() && _root->name != 0 );  //Node has a name.
	}
	bool has_name( const String& name ) const {
		return has_name( name.c_str() );  //Node is named 'name'.
	}
	bool has_attribute( const String& name ) const {
		return has_attribute( name.c_str() );  //Node has an attribute named 'name'.
	}
#ifdef PUGOPT_NONSEG
	bool has_name( const TCHAR* name ) const {
		return ( name && _root && _root->name && _tcsncmp( _root->name, name, _root->name_size ) == 0 );    //Node is named 'name'.
	}
#else
	bool has_name( const TCHAR* name ) const {
		return ( name && _root && _root->name && strcmpwild( name, _root->name ) == 0 );    //Node is named 'name'.
	}
#endif
	bool has_attribute( const TCHAR* name ) const {
		return ( mapto_attribute_idx( name ) > -1 );    //Node has an attribute named name.
	}
...
public:
	...
	unsigned int name_size() const {
		return ( has_name() ) ? _root->name_size : 0;  //Get node name length if any, else 0.
	}
	unsigned int value_size() const {
		return ( has_value() ) ? _root->value_size : 0;  //Get node value length if any, else 0.
	}
	...
	inline bool matches_attribute_name( const TCHAR* name, const unsigned int namelen, const int i ) const {
		return ( _tcsncmp( name, _root->attribute[i]->name, max( namelen, _root->attribute[i]->name_size ) ) == 0 );    //There is an attribute at 'i' named 'name'.
	}
	inline bool matches_child_name( const TCHAR* name, const unsigned int namelen, const int i ) const {
		return ( _tcsncmp( name, _root->child[i]->name, max( namelen, _root->child[i]->name_size ) ) == 0 );    //There is a child at 'i' named 'name'.
	}
	inline bool matches_name( const TCHAR* name, const unsigned int namelen, xml_node_struct* node ) const {
		return ( _tcsncmp( name, node->name, max( namelen, node->name_size ) ) == 0 );    //This is named 'name'.
	}
	inline bool matches_value( const TCHAR* data, const unsigned int datalen, xml_node_struct* node ) const {
		return ( _tcsncmp( data, node->value, max( datalen, node->value_size ) ) == 0 );    //This is valued 'value'.
	}
	inline bool matches_attribute_name( const TCHAR* name, const unsigned int namelen, xml_attribute_struct* attr ) const {
		return ( _tcsncmp( name, attr->name, max( namelen, attr->name_size ) ) == 0 );    //The given attribute is named 'name'.
	}
	...
	inline bool matches_attribute_name_value( const TCHAR* value, const unsigned int valulen, xml_attribute_struct* attr ) const {
		return attr->matches_attribute_name_value( value, valulen ); //The given attribute is valued 'value'.
	}
#else
	const TCHAR* name() {
		return ( has_name() ) ? _root->name : _T( "" );    //Access pointer to node name if any, else empty string.
	}
	inline bool matches_attribute_name( const TCHAR* name, const unsigned int i ) const {
		return ( strcmpwild( name, _root->attribute[i]->name ) == 0 );    //There is an attribute at 'i' named 'name'.
	}
	inline bool matches_child_name( const TCHAR* name, const unsigned int i ) const {
		return ( strcmpwild( name, _root->child[i]->name ) == 0 );    //There is a child at 'i' named 'name'.
	}
	inline bool matches_name( const TCHAR* name, xml_node_struct* node ) const {
		return ( strcmpwild( name, node->name ) == 0 );    //This is named 'name'.
	}
	inline bool matches_value( const TCHAR* data, xml_node_struct* node ) const {
		return ( strcmpwild( data, node->value ) == 0 );    //This is valued 'value'.
	}
	inline bool matches_attribute_name( const TCHAR* attribute, xml_attribute_struct* attr ) const {
		return ( strcmpwild( attribute, attr->name ) == 0 );    //The given attribute is named 'name'.
	}
// inline bool matches_attribute_name_value(const TCHAR* value,xml_attribute_struct* attr) const { return (strcmpwild(value,attr->value)==0); } //The given attribute is valued 'value'.
	inline bool matches_attribute_name_value( const TCHAR* value, xml_attribute_struct* attr ) const {
		return attr->matches_attribute_name_value( value );  //The given attribute is valued 'value'.
	}
#endif
	xml_node_type type() const {
		return ( _root ) ? ( xml_node_type )_root->type : node_null;    //Access node entity type.
	}
	const TCHAR* value() const {
		return ( has_value() ) ? _root->value : _T( "" );    //Access pointer to data if any, else empty string.
	}

	//<summary>Get node name as const String. If no name, return empty.</summary>
	//<returns>The String node name, or empty string.</returns>
	...
	//<summary>Get node value as const String. If no value, return empty.</summary>
	//<returns>The String node value, or empty string.</returns>
	//<since>NF 24 Mar 2003</since>
	...
	unsigned int children() const {
		return _root->children;    //Access node's child count.
	}
	xml_node child( unsigned int i ) {
		return ( i < children() ) ? xml_node( _root->child[i] ) : xml_node();    //Access child node at subscript as xml_node or xml_node(NULL) if bad subscript.
	}
	unsigned int attributes() const {
		return _root->attributes;    //Access node's attribute count.
	}
	xml_attribute attribute( unsigned int i ) {
		return ( i < attributes() ) ? xml_attribute( _root->attribute[i] ) : xml_attribute();    //Access attribute at subscript if any, else empty attribute.
	}
	...
	xml_attribute attribute( const String& name ) {
	...
	const unsigned int siblings() const {
		return ( !type_document() ) ? _root->parent->children : 0;  //Access node's sibling count (parent's child count).
	}
	xml_node sibling( unsigned int i ) {
		return ( !type_document() && i < siblings() ) ? xml_node( _root->parent->child[i] ) : xml_node();    //Access sibling node at subscript as xml_node or xml_node(NULL) if bad subscript.
	}
	xml_node parent() const {
		return ( !type_document() ) ? xml_node( _root->parent ) : xml_node();    //Access node's parent if any, else xml_node(NULL)
	}
...
#else
					    min(( unsigned int )valuelen, ( unsigned int )_tcslen( node->value ) );
#endif
...
public:
	...
	xml_attribute_struct* mapto_attribute_ptr( const String& name ) const {
	...
	xml_node_struct* mapto_child_ptr( const String& name ) const {
	...
	//<returns>Index of child, or -1 if not found.</returns>
	//<remarks>Implement your own hash table if you have a great many children.</remarks>
	int mapto_child_idx( const String& name ) const {
...
public:
	...
	void all_elements_by_name( const String& name, pointer_array& found ) const {
	...
	xml_node first_element_by_name( const String& name, const xml_node_type type = node_null ) const {
		return first_element_by_name( name.c_str(), type );
	}
	...
	//<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
	xml_node first_element_by_value( const String& name, const String& value ) const {
		return first_element_by_value( name.c_str(), value.c_str() );
	}
	...
	//<returns>Valid xml_node if such element named 'name' is found.</returns>
	//<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
	xml_node first_element_by_attribute( const String& name, const String& attr_name, const String& attr_value, const bool bDescend = true )const {
		return first_element_by_attribute( name.c_str(), attr_name.c_str(), attr_value.c_str(), bDescend );
	}
	...
	//<remarks>If matching co-node was found, '_root' points thereto.</remarks>
	bool moveto_first_sibling( const String& name ) {
	...
	//<remarks>If matching sub-node was found, '_root' points thereto.</remarks>
	bool moveto_child( const String& name ) {
		return moveto_child( name.c_str() );
	}
	...
	bool moveto_next_sibling( const String& name ) {
	...
	String path( const TCHAR* delimiter = _T( "/" ) )
	{
		...
		strcatgrow( &temp, path ); //Append current path.
		free( path ); //Free the old path.
		String returns = temp; //Set path as new string.
		free( temp );
	...
	String pathaddress( const TCHAR* delimiter = _T( "/" ) )
	{
		xml_node cursor = *this; //Make a copy.
		TCHAR szTemp[ 50 ];
		String spath( _itot(( int )( xml_node_struct* )cursor, szTemp, 16 ) );
		while ( cursor.moveto_parent() && !cursor.type_document() ) //Loop to parent (stopping on actual root because it has no name).
		{
			_itot(( int )( xml_node_struct* )cursor, szTemp, 16 );
			spath = String( szTemp ) + delimiter + spath;
		}
	...
	xml_node first_element_by_path( const String& path, const String& delimiter = _T( "/" ) ) {
		return first_element_by_path( path.c_str(), delimiter.c_str() );
	}
...
public:
	...
	//<returns>Success.</returns>
	bool value( const String& new_value ) {
		return value( new_value.c_str() );
	}
	...
	bool remove_attribute( const String& name ) {
		return remove_attribute( name.c_str() );
	}
	...
	xml_attribute append_attribute( const String& name, const String& value ) {
	...
	xml_node set_child_value( xml_node_type type, const String& new_value )
	{
	...
	//<since>4 Mar 2003 NF</since>
	xml_node append_child( xml_node_type type, const String& new_name )
	{
...
public:
	...
	String xml_to_text( const String& sxml_pcdata ) const
	{
		assert( type_doctype() );  // must be DTD - see cmt above.
		String sxml_value( sxml_pcdata );
		String::size_type idx = 0;
		do  // parse sxml_value
		{       // find an entity reference "&...;"
			idx = sxml_value.find( '&', idx );
			String::size_type idxe = sxml_value.find( ';', idx );
			if ( idx != String::npos
			        && idxe != String::npos
			   )
			{       // get the text in between "&...;"
				const int len = ( int )( idxe - idx ) - 1;
				String token = sxml_value.substr( ++idx, len );  // rem: incr past the '&'
				...
				{       // get the entity value, stripping off the surrounding quotes.
					String entvalue = dtdentity.get_value().substr( 1, dtdentity.get_value().length() - 2 );
					token = entvalue;
					...
					if ( token.compare( 0, 2, _T( "&#" ) ) == 0 )
					{
						...
						const String::size_type idxsc = token.find( ';' );      // see if > 1 ";"
						if ( idxsc == String::npos || idxsc == token.length() - 1 )
						{
...
public:
	...
	operator xml_node_struct*() {
		return _xmldoc;    //Cast as xml_node_struct pointer to root.
	}
	operator xml_node() {
		return xml_node( _xmldoc );  //Cast as xml_node (same as document).
	}
	xml_node document() {
		return xml_node( _xmldoc );  //Returns the root wrapped by an xml_node.
	}
...
public:
	xml_node at( long i ) {
		return xml_node(( xml_node_struct* )pointer_array::at(( unsigned int )i ) );    //Access xml_node at subscript.
	}
	xml_node operator[]( long i ) {
		return xml_node(( xml_node_struct* )pointer_array::at(( unsigned int )i ) );    //Access xml_node at subscript.
	}
